...
nfeat <- min(3000, max(2, nrow(tmdata)))
tmdata <- FindVariableFeatures(tmdata, nfeatures = nfeat, verbose = FALSE)

# scale only variable features to reduce memory pressure
tmdata <- ScaleData(tmdata, features = VariableFeatures(tmdata), verbose = FALSE)

max_pcs <- max(1, min(length(VariableFeatures(tmdata)), ncells - 1))
...
# cap k to avoid huge kNN graphs
if (ncells <= 10) {
...
# use Louvain (algorithm=1) to avoid Leiden long-vector crash
if (ncells >= 3) {
  tmdata <- FindClusters(tmdata, resolution = resolution, algorithm = 1, verbose = FALSE)
}
...
saveRDS(tmdata, paste0("by_samples/", sample, "/", sample, "_initial.rds"))