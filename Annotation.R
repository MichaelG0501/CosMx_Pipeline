...
sample_dirs <- sample_dirs[grepl("^[^/]+_[^/]+$", sample_dirs)]  # match *_*_*
...
expr_home_threshold  <- 20   # per-sample for HOME celltype
home_min_pct_samps   <- 50   # % of samples (home) that must pass
off_max_pct_samps    <- 50   # % of samples (off) that may pass
...
for (sample in sample_dirs) {
  ...
  tmdata@meta.data[mod_cols] <- lapply(tmdata@meta.data[mod_cols], function(x) {
    ifelse(x >= 1, 1,
